...
def main():
    ...
    if config.train.resume_path is not None:
        model_state_file = os.path.join(config.train.resume_path,
                                        config.pretrained if config.pretrained is not None 
                                        else 'checkpoint.pth.tar')
        if os.path.isfile(model_state_file):
            print('\n Loading model from: ', model_state_file)
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if ((epoch+1) % bisect_right(config.train.val_span, -epoch) == 0) or config.val_each_epoch:
            valid_loss, mae, mse, nae, ppmae = validate(config,
                                                 testloader, model, writer_dict, device,
                                                 config.test.patch_batch_size,
                                                 train_log_dir + '/../val', test_dataset.mean,
                                                test_dataset.std)
            if config.log:
                wandb.log({"Val_MAE": mae, "VAL_mse": mse, "VAL_nae" : nae, "PPMAE":ppmae })
            ...
            if args.local_rank == 0:
                ...
                if mse < best_mse:
                    ...
                    torch.save(
                        ...
                        os.path.join(
                            ...
                            str(mse) +
                            '.pth'))
                logger.info('=> I am saving checkpoint to {}'.format(
                    train_log_dir + 'checkpoint'+str(mae)+'_'+str(mse)+'.pth.tar'))
                torch.save({
                    'epoch': epoch + 1,
                    'best_MAE': best_mae,
                    'best_MSE': best_mse,
                ...
                }, os.path.join(train_log_dir, 'checkpoint'+str(mae)+'_'+str(mse)+'.pth.tar'))
