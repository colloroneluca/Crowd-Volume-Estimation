...
def main():
    ...
    logger.info('GPU idx: 0 ' )
    save_count_txt = './logs.txt'
    gpus = config.gpus
    distributed = False #torch.cuda.device_count() > 1
    if distributed:
    ...
    print(f'This model has {pytorch_total_params} parameters')
    # prepare data
    testing = True
    print('Testing on ',config.dataset.test_set,'\n')
    test_dataset = eval('datasets.' + config.dataset.name)(
        ...
        phase='test',
        testing=testing,
        temporal = config.dataset.temporal)
    ...
    if 'test' in config.dataset.test_set:
        ...
        mae, mse, nae,save_count_txt, ppmae = test_cc(config, test_dataset, testloader, model,
