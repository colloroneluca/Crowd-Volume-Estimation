...
class BaseDataset(data.Dataset):
    ...
    def center_crop(self, image, label):
        ...
        return image, label
    def image_points_resize(self, image, long_size, label=None):
        h, w = image.shape[:2]
        ratio = 1.
    ...
    def multi_scale_aug(self, image, label=None, 
        ...
        long_size = np.int64(self.base_size * rand_scale + 0.5)
        if label is not None:
    ...
    def gen_sample(self, image, points,
        ...
        if multi_scale:
            ...
            image, label = self.multi_scale_aug(image, points,
    ...
    def multi_scale_inference(self, model, image, scales=[1], flip=False):
        ...
        stride_h = np.int64(self.crop_size[0] * 2.0 / 3.0)
        stride_w = np.int64(self.crop_size[1] * 2.0 / 3.0)
